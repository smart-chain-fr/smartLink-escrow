<template>
  <v-navigation-drawer v-model="drawer" floating app>
    <v-list-item>
      <v-list-item-content>
        <v-img :src="require('../../assets/full_logo.svg')"> </v-img>
        <v-list-item-subtitle>
          Buy, Sell, Swap goods on your terms
        </v-list-item-subtitle>
      </v-list-item-content>
    </v-list-item>
    ...
    <v-list dense nav>
      ...
      >
        ...
        <v-list-item-content>
          <v-list-item-title :key="nbNew" v-if="item.name === 'offers' && nbNew > 0">
             <v-row> <v-col cols=12 lg=4>
            {{ item.title }}
            </v-col>
            <v-col cols=12 lg=8>
            <div class="notification"> {{ nbNew}} </div>
            </v-col></v-row>
            </v-list-item-title
          >
